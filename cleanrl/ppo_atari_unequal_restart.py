...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            teacher_next_obs, teacher_next_done = torch.Tensor(teacher_next_obs).to(device), torch.Tensor(teacher_next_done).to(device)
            student_next_obs, student_next_done = torch.Tensor(student_next_obs).to(device), torch.Tensor(student_next_done).to(device)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.detach:
                    teacher_loss += args.alpha * kl_div_logits(teacher_logits, student_logits.detach(), args.T)
                else:
                    teacher_loss += args.alpha * kl_div_logits(teacher_logits, student_logits, args.T)

                student_loss = student_pg_loss + student_v_loss * args.vf_coef
                if args.detach:
                    student_loss += args.student_alpha * kl_div_logits(student_logits, teacher_logits.detach(), args.T)
                else:
                    student_loss += args.student_alpha * kl_div_logits(student_logits, teacher_logits, args.T)
